...
async def update_submission_verdict(
    db, submission_id: str, verdict_type: str, score: float
) -> bool:
    stmt = select(Submission).where(Submission.id == submission_id)
    result = await db.execute(stmt)
    submission = result.scalar_one_or_none()
...
async def load_submissions_by_user_and_problem(
    db, username: str, problem_id: str
) -> list[SubmissionInfo]:
    stmt = (
        select(Submission)
        .where(
            (Submission.username == username) & (Submission.problem_id == problem_id)
        )
