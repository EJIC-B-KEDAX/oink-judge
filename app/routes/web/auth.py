...
from app.services.auth.auth_api import delete_account, login, logout, register
...
async def handle_register(
    ...
    if not re.fullmatch(r"[a-zA-Z0-9.\-_?!()]{3,100}", username):
        return templates.TemplateResponse(
            ...
            {
                "request": request,
                "error": "invalid characters in username, check pattern is [a-zA-Z0-9.-_?!()]{3,100}",
            },
        )
...
async def show_delete_account_page(
    request: Request, username: str = Depends(require_current_user)
):
    return templates.TemplateResponse(
        "delete_account.html", {"request": request, "username": username}
    )
...
async def handle_delete_account(
    ...
    return templates.TemplateResponse(
        "delete_account.html", {"request": request, "error": "Account deletion failed"}
    )
...
async def handle_login(
    ...
    if auth_response.get("status") == "error":
        return templates.TemplateResponse(
            "login.html", {"request": request, "error": auth_response["message"]}
        )
    else:
