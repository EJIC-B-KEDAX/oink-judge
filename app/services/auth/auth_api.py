...
auth_connection_event_handler = AuthorizingEventHandler(
...
async def whose_session(session_id: str) -> str | None:
    ...
    request = {"request": "whose_session", "session_id": session_id}
    response = await connection.send_request(request)
...
async def register(username: str, password: str) -> bool:
    ...
    request = {"request": "register", "username": username, "password": password}
    response = await connection.send_request(request)
...
async def delete_account(username: str) -> bool:
    ...
    request = {"request": "delete_user", "username": username}
    response = await connection.send_request(request)
    return response["status"] == "success"
...
async def logout(session_id: str) -> bool:
    ...
    request = {"request": "logout", "session_id": session_id}
    response = await connection.send_request(request)
    return response["status"] == "success"
