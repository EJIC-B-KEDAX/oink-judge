...
awaitable<void> AuthRequiredProtocol::start(const std::string &start_message) {
...
awaitable<void> AuthRequiredProtocol::receive_message(const std::string &message) {
    if (_status == AUTHORIZED) {
        ...
        if (message == _auth_token) {
            _status = AUTHORIZED;
            co_await ProtocolDecorator::start(_saved_start_message);
