...
auto VerdictBuilderMin::addVerdict(std::shared_ptr<DefaultVerdict> verdict) -> void {
    ...
    if (current_info.type.type == VerdictType::Type::ACCEPTED_T) {
        ...
        if (info.type.type == VerdictType::Type::ACCEPTED_T) {
            new_type = current_info.type;
        } else {
            new_type = info.type;
        }
    } else if (current_info.type.type == VerdictType::Type::SKIPPED_T) {
        if (info.type.type == VerdictType::Type::FAILED_T || info.type.type == VerdictType::Type::WRONG_T ||
            info.type.type == VerdictType::Type::PARTIAL_T) {
            new_type = info.type;
        } else {
            new_type = current_info.type;
        }
