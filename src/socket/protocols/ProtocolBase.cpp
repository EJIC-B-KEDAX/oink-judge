...
void ProtocolBase::send_message(const std::string &message) {
        get_session()->send_message(message);
}
...
void ProtocolBase::request_internal(const std::string &message, const callback_t &callback) {
    throw std::runtime_error("Request is not inmplemented for this protocol");
}
