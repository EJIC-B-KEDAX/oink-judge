...
ProblemTable::ProblemTable(const std::string& problem_id) : id_(problem_id) {
    touch_table();
    ...
    const std::string set_tested_on_revision_sql =
        "UPDATE " + get_table_name() + " SET tested_on_revision = $2 WHERE username = $1";
...
std::string ProblemTable::get_table_name() const { return DataBase::instance().quote_table("problem_" + id_); }

std::string ProblemTable::get_raw_table_name() const { return "problem_" + id_; }
...
void ProblemTable::set_score_on_test(const std::string& username, std::shared_ptr<Test> test, double score) {
    add_new_user(username);

    std::string column_name = DataBase::instance().quote_name("score_on_" + test->get_name());
...
void ProblemTable::add_new_user(const std::string& username) { // NOLINT(readability-make-member-function-const)
    if (does_user_have_submission(username)) {
        return;
    }
...
double ProblemTable::get_score_on_test(const std::string& username, std::shared_ptr<Test> test) const {
    std::string column_name = DataBase::instance().quote_name("score_on_" + test->get_name());
...
std::vector<std::string> ProblemTable::get_right_columns() const {
    ...
    std::vector<std::string> test_names = problem_config::getAllTestNames(id_);
...
void ProblemTable::touch_table() { DataBase::instance().execute_sql(generate_create_sql(get_table_name(), get_right_columns())); }
