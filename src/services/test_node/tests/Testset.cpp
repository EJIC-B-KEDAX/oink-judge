...
Testset::Testset(ProblemBuilder* problem_builder, const std::string& problem_id, const std::string& testset_name)
    : _name(testset_name) {
    pugi::xml_node testset_config = problem_config::getTestConfig(problem_id, testset_name);
...
std::shared_ptr<Verdict> Testset::run(const std::string& submission_id, const std::vector<std::string>& boxes,
    ...
    for (size_t i = 0; i < _tests.size(); ++i) {
        ...
        if (!_verdict_builder->can_score_change())
            test_verdict = _tests[i]->skip(submission_id);
        else
            test_verdict = _tests[i]->run(submission_id, boxes, additional_params);
...
const std::string& Testset::get_name() const { return _name; }
