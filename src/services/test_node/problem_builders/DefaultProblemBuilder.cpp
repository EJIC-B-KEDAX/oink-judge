...
std::shared_ptr<Test> DefaultProblemBuilder::build() {
    std::vector<std::string> test_names = problem_config::getAllTestNames(_problem_id);

    for (const auto& test_name : test_names) {
        pugi::xml_node test_config = problem_config::getTestConfig(_problem_id, test_name);
